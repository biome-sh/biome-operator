...
import (
	...
	habclientset "github.com/biome-sh/biome-operator/pkg/client/clientset/versioned"
	habinformers "github.com/biome-sh/biome-operator/pkg/client/informers/externalversions"
	habv1beta2controller "github.com/biome-sh/biome-operator/pkg/controller/v1beta2"
	"github.com/biome-sh/biome-operator/pkg/version"
)
...
func run() int {
	...
	// This is the clientset for interacting with the Biome API.
	habClientset, err := bioclientset.NewForConfig(config)
	if err != nil {
		level.Error(logger).Log("msg", err)
		return 1
	}
...
// createCRD creates Biome CRD in the cluster, provided the operator has 'create'
...
func createCRD(cSets Clientsets, logger log.Logger) error {
	if _, err := biov1beta2controller.CreateCRD(cSets.ApiextensionsClientset); err != nil {
		if !apierrors.IsAlreadyExists(err) {
			return errors.Wrap(err, "create Biome CRD failed")
		}
		level.Info(logger).Log("msg", "Biome CRD already exists, continuing")
		return nil
	}
	level.Info(logger).Log("msg", "created Biome CRD")
	return nil
}
...
func v1beta2(ctx context.Context, wg *sync.WaitGroup, cSets Clientsets, logger log.Logger, flags *FlagOpts) error {
	...
	habInformerFactory := bioinformers.NewSharedInformerFactoryWithOptions(
		cSets.HabClientset,
		resyncPeriod,
		habinformers.WithNamespace(flags.Namespace),
	)

	config := biov1beta2controller.Config{
		...
		BiomeClient:          cSets.HabClientset.BiomeV1beta1().RESTClient(),
		KubernetesClientset:    cSets.KubeClientset,
		KubeInformerFactory:    kubeInformerFactory,
		BiomeInformerFactory: bioInformerFactory,
		Namespace:              flags.Namespace,
	}
	controller, err := biov1beta2controller.New(config, log.With(logger, "component", "controller/v1beta2"))
	if err != nil {
		return err
	}
